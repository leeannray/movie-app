...
if(typeof parsedUrl.query==='string'){parsedUrl.query=(0,_querystring.parse)(parsedUrl.query);}const{basePath}=this.nextConfig;if(basePath&&((_req$url=req.url)==null?void 0:_req$url.startsWith(basePath))){// store original URL to allow checking if basePath was
// provided or not
...
if(isDataReq&&urlPathname.includes(this.buildId)){urlPathname=(0,_denormalizePagePath.denormalizePagePath)((urlPathname.split(this.buildId).pop()||'/').replace(/\.json$/,''));}const ssgCacheKey=isPreviewMode||!isSSG?undefined// Preview mode bypasses the cache
...
if(isLikeServerless){renderResult=await components.Component.renderReqToHTML(req,res,'passthrough',{fontManifest:this.renderOpts.fontManifest});html=renderResult.html;pageData=renderResult.renderOpts.pageData;sprRevalidate=renderResult.renderOpts.revalidate;}else{const renderOpts={...components,...opts,isDataReq,resolvedUrl:(0,_url.format)({pathname:urlPathname,// make sure to only add query values from original URL
query:(0,_url.parse)(req.url||'',true).query})};renderResult=await(0,_render.renderToHTML)(req,res,pathname,query,renderOpts);html=renderResult;// TODO: change this to a different passing mechanism
